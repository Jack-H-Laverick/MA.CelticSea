...
labelled <- st_intersection(Edges, grid) %>% 
  ...
  characterise_flows(st_transform(domains, crs = st_crs(Edges)), precision = 1000) %>%                                               # In which direction? (in or out of box and with which neighbour)
...
shallow <- mutate(labelled,
                  thickness = ifelse(Bathymetry > 400, 340, Bathymetry-60),
